...
const HandleGetAssignmentsByBranch = async (req, res) => {
  try {
    ...
    const assignments = await Assignment.find({ branchID })
      .populate({
        path: "branchID",
        model: "branches",
      })
...
const HandleUpdateAssignmentStatus = async (req, res) => {
  try {
    ...
    const uploadResult = completionPicture
      ? await cloudinary.uploader.upload(completionPicture.tempFilePath, {
          resource_type: "image",
    ...
    if (status.includes("Order Assigned")) {
      ...
      findAssignment.transferredFromRiders = [
        ...findAssignment.transferredFromRiders,
        findAssignment.riderID,
      ];
    ...
    if (
      status.includes("Shipment Collected") &&
      findParcel.status.includes("pending")
    ) {
    ...
    if (status.includes("Delivered")) {
      if (
        findAssignment.Status.includes("Out for Delivery") ||
        findAssignment.Status.includes("Shipment Sorted at Delivery Facility")
      ) {
...
const HandleTransferAssignment = async (req, res) => {
  try {
    ...
    if (
      !findAssignment.transferredFromRiders.includes(findAssignment.riderID)
    ) {
      findAssignment.transferredFromRiders = [
        ...findAssignment.transferredFromRiders,
        findAssignment.riderID,
      ...
      return res
        .status(200)
        .json({ message: "Assignment Transfer Successful" });
    } else {
      return res
        .status(400)
        .json({ message: "Driver has already transferred this assignment" });
    }
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal Server Error" });
...
const HandleGetRiderJobs = async (req, res) => {
  try {
    ...
    const findRider =
      (await RiderSchema.findById(riderID)) ||
      (await UserSchema.findById(riderID)) ||
      (await SuperAdmin.findById(riderID));
    ...
    if (findRider.role.includes("Rider")) {
      if (isHistory === "") {
        const assignments = await Assignment.find({
          ...
          model: "parcel",
          select:
            "Status status _id haveOwnTrackID ownTrackID parcelDescription rateListID reciverAddress SenderAddress CodCharges CodAmount isDamaged weight Dimension",
        });
        ...
        const assignments = await Assignment.find({
          ...
          model: "parcel",
          select:
            "Status status _id haveOwnTrackID ownTrackID parcelDescription rateListID reciverAddress SenderAddress CodCharges CodAmount isDamaged weight",
        });
        ...
        const assignments = await Assignment.find({
          ...
          model: "parcel",
          select:
            "Status status  haveOwnTrackID ownTrackID parcelDescription rateListID reciverAddress SenderAddress CodCharges CodAmount isDamaged weight",
        });
      ...
      const mapParcels = findParcels.map(async (parcel) => {
        const mapAssignments = await Assignment.find({
          parcelID: parcel._id,
        }).populate({
          path: "parcelID",
          model: "parcel",
        ...
        if (mapAssignments.length === 0) {
          ...
          const filteredRateList = findRateList
            ? findRateList.rateList.filter(
                (rate) => rate._id.toString() === parcel.rateListID.toString()
              )
            : [];
...
const HandleAcceptJob = async (req, res) => {
  try {
    const { assignmentID, riderID } = req.params;
...
const HandleUserCancelParcelID = async (req, res) => {
  try {
    ...
    if (!findParcel.status.includes("pending")) {
      return res
        .status(403)
        .json({ message: "Contact Your Manager To Cancel Your Parcel" });
    }
    ...
    if (status.includes("Cancelled")) {
      ...
      return res
        .status(200)
        .json({ message: "Parcel Status Updated Successfully" });
    } else {
